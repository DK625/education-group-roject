const {
    CONFIG_MESSAGE_ERRORS, PAYMENT_TYPES, CONTEXT_NOTIFICATION, ACTION_NOTIFICATION_ORDER,
} = require("../configs");
...
const {
    pushNotification, getUserAndAdminTokens,
        ...
        try {
            ...
            if (newData.length) {
                ...
                const dataCreate = {
                    ...
                    }, itemsPrice, shippingPrice, totalPrice, user: user, isDelivered: false, isPaid, paidAt,
                };
                if (deliveryMethod) {
...
const deleteOrderProduct = (id) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await Promise.all(checkOrder.orderItems.map(async (orderItem) => {
                const product = await Product.findById(orderItem.product);
                if (product) {
                    await Product.findByIdAndUpdate(orderItem.product, {
                        $inc: {
                            countInStock: orderItem.amount, sold: -orderItem.amount,
                        },
                    });
                }
            }));
...
const updateOrder = (id, data) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            existingOrder.shippingAddress = data.shippingAddress || existingOrder.shippingAddress;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const fieldsToSelect = {
                ...
                orderItems: 1, shippingAddress: 1, paymentMethod: 1, // deliveryMethod: 1,
                // itemsPrice: 1,
                // shippingPrice: 1,
                totalPrice: 1, // user: 1,
                isPaid: 1, // paidAt: 1,
                deliveryAt: 1, isDelivered: 1,
                createdAt: 1
            };
            ...
            if (page === -1 && limit === -1) {
                const allOrder = await Order.find(query)
                    ...
                    .populate({
                        path: "shippingAddress.city", select: "_id name", // Select only the 'slug' field of the product
                    });
                ...
                resolve({
                    ...
                    statusMessage: "Success",
                    data: {
                        orders: allOrder, totalPage: 1, totalCount: totalCount,
                    },
                });
                return;
            }
            ...
            // const allOrder = await Order.find({status: 1})
            ...
            resolve({
                ...
                statusMessage: "Success",
                data: {
                    orders: allOrder, totalPage: totalPage, totalCount: totalCount,
                },
            });
        } catch (e) {
            reject(e);
...
const getAllOrderOfMe = (userId, params) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const fieldsToSelect = {
                status: 1, // createdAt: 1,
                orderItems: 1, shippingAddress: 1, // paymentMethod: 1,
                // deliveryMethod: 1,
                // itemsPrice: 1,
                // shippingPrice: 1,
                totalPrice: 1, // user: 1,
                isPaid: 1, paidAt: 1, deliveryAt: 1, isDelivered: 1,
            };
            const allOrder = await Order.find(query)
                .skip(startIndex)
                .limit(limit)
                .sort(sortOptions)
                .populate([{
                    path: "user", select: "_id firstName lastName middleName",
                }, {
                    path: "deliveryMethod", select: "name price",
                }, {
                    path: "paymentMethod", select: "name type",
                }, {
                    path: "orderItems.product", select: "countInStock slug", // Select only the 'slug' field of the product
...
const cancelOrderProductByAdmin = async (orderId) => {
    try {
